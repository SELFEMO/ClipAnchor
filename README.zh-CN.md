...
| Linux x64     | 待验证 | 仍需 DEB/RPM 安装包与桌面环境验证。 |
...
必须在克隆后的 `ClipAnchor` 目录内执行 `npm install` 和 `npm run desktop:dev`。如果在上级目录执行，会因为找不到 `package.json` 出现 `Could not read package.json` 错误。需要干净重建时请使用 `npm run clean`，不要手动拼接 `rm -rf` 清理命令，可避免 macOS 偶发的 `Directory not empty`。Windows 开发环境建议先安装 Rust、Node.js、Microsoft Visual Studio Build Tools 和 WebView2 Runtime。项目根目录包含 `.cargo/config.toml`，Cargo 默认使用 sparse 镜像源，适合网络访问 crates.io 不稳定的环境。
...
在 macOS 主机上构建 Apple Silicon 包时，先添加目标并执行 ARM64 构建：
...
Tauri 会把安装包输出到 `src-tauri/target/release/bundle/`；指定 target 时会输出到 `src-tauri/target/<target-triple>/release/bundle/`。项目脚本会把可分发产物复制到根目录 `release/`，便于查找和发布。

Linux 构建目标为 DEB 和 RPM。Windows 构建目标包含 NSIS 安装器和 MSI 安装包。macOS 构建目标包含 APP 和 DMG。macOS DMG 建议在 macOS 上生成，公开分发前使用自己的 Apple Developer 证书签名并公证。
...
设置中开启 **自动更新** 后，ClipAnchor 启动时会在后台静默检查 GitHub Releases。启动检查处于检查中、无更新、普通失败、或云端新 tag 没有当前平台兼容安装包时，都不会自动弹出更新卡片。只有存在可操作更新时才会提示，例如兼容安装包已下载完成，或存在兼容安装包但自动下载失败，需要用户决定是否打开发布资产。

用户点击 **检查更新** 按钮后会立即进入应用内状态卡片，并展示正在检查、正在下载、准备安装、已是最新、没有兼容安装包或失败等状态。发布标签建议使用 `pre-release-v...` 或 `release-v...`。更新包会自动选择：Windows 优先使用 `ClipAnchor_Windows_x64.exe`；如果没有 EXE，则选择匹配系统语言的 MSI，例如 `ClipAnchor_Windows_x64_zh-CN.msi` 或 `ClipAnchor_Windows_x64_en-US.msi`。macOS 使用 DMG，并会根据文件名过滤架构，Apple Silicon 会优先选择 ARM64 或 universal 包，不会误选 Intel-only 包。Linux 根据发行版家族选择 DEB 或 RPM。每次重新检查前会先清理 `data/updates/` 中的旧安装包，避免多个历史版本残留导致误用。下载完成的新安装包存放在 `data/updates/`，用户点击 **立即更新** 后由系统安装器打开。
