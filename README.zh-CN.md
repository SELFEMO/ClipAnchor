...
| 快捷键 | 支持置顶服务、历史服务、显示/隐藏主界面、轻量模式和暂停/恢复监听等全局操作。 |
...
3. 安装软件或解压便携包。
4. 启动 ClipAnchor，并在设置中配置快捷键、隐私过滤、外观、语言和数据清理策略。
...
5. 在设置页面调整主题、语言、缩放、快捷键、弹窗位置、隐私过滤、自动销毁时间和数据清理策略。
...
随后完全退出 ClipAnchor，包括托盘进程，再重新启动。该语言会出现在设置中的扩展语言列表内。
...
只有存在可操作更新时才会提示，例如兼容安装包已经准备完成，或存在兼容安装包但自动下载失败，需要用户决定是否打开发布资产。
...
每次重新检查前会先清理 `data/updates/` 中的旧安装包，避免历史版本被误用。下载完成的新安装包存放在 `data/updates/`，用户点击 **立即更新** 后由系统安装器打开。
