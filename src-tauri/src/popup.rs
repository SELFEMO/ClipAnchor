...
fn build_popup_window(app: &AppHandle, state: &AppState, item: &ClipItem, settings: &AppSettings, pinned: bool) -> Result<(), String> {
    ...
    delayed_show_popup(app, &popup_label(&item.id), pinned);
    Ok(())
}

fn delayed_show_popup(app: &AppHandle, label: &str, pinned: bool) {
    ...
    thread::spawn(move || {
        ...
        let _ = handle.run_on_main_thread(move || {
            if let Some(window) = ui_handle.get_webview_window(&label) {
...
mod native_popup_shape {
    ...
    #[link(name = "gdi32")]
    unsafe extern "system" {
        fn CreateRoundRectRgn(left: i32, top: i32, right: i32, bottom: i32, ellipse_width: i32, ellipse_height: i32) -> NativeRegion;
    ...
    pub fn apply<R: Runtime>(window: &WebviewWindow<R>) {
        ...
        apply_exact_window_region(hwnd);
    }
    ...
    fn disable_dwm_extra_corner(hwnd: HWND) {
        ...
        // 先关闭 DWM 自动圆角，是为了让 SetWindowRgn 成为唯一裁剪来源，避免系统圆角和前端圆角叠成两层边界。
        // DWM auto corners are disabled first so SetWindowRgn becomes the only clipping source and does not stack with frontend corners.
        let _ = unsafe {
    ...
    fn apply_exact_window_region(hwnd: HWND) {
        ...
        let scale = dpi / 96.0;
        let radius = (30.0 * scale).round().clamp(18.0, 72.0) as i32;
        let diameter = radius * 2;
        let region = unsafe { CreateRoundRectRgn(0, 0, width + 1, height + 1, diameter, diameter) };
        if region.is_null() {
            return;
        }
        // Region 直接裁掉原生窗口的直角底板，是为了即使 WebView2 透明层失效也不会露出白色直角背景。
        // The region clips the native square base directly so no white square background appears even when WebView2 transparency fails.
        if unsafe { SetWindowRgn(hwnd, region, 1) } == 0 {
            let _ = unsafe { DeleteObject(region as *mut c_void) };
            return;
        }
        // 强制刷新非客户区，是为了让 Windows 立即丢弃旧的直角阴影/边框缓存。
        // The non-client area is redrawn immediately so Windows drops any cached square shadow or border.
        let _ = unsafe { RedrawWindow(hwnd, std::ptr::null(), std::ptr::null_mut(), RDW_INVALIDATE | RDW_FRAME | RDW_UPDATENOW) };
    }
}

#[cfg(target_os = "windows")]
fn apply_native_popup_shape(window: &tauri::WebviewWindow) {
    native_popup_shape::apply(window);
}

#[cfg(not(target_os = "windows"))]
fn apply_native_popup_shape(_window: &tauri::WebviewWindow) {}
