...
fn poll_once(app: &AppHandle, state: &AppState, last_signature: &mut String) -> Result<(), String> {
    ...
    if settings.clipboard_paused {
        // 暂停监听时既不写库也不弹窗，是为了让输入密码等敏感操作有明确的临时关闭入口。
        // Pause stops both database writes and popups so password entry and similar sensitive work have an explicit off switch.
        return Ok(());
    }
...
fn process_item(app: &AppHandle, state: &AppState, settings: &AppSettings, item: ClipItem) -> Result<(), String> {
    ...
    if crate::privacy::should_filter_sensitive(settings, &item) {
